...
class BookModel:
    ...
    def create_book(self, title: str, author: str, year: int, price: float) -> str:
        try:
            result = self.collection.insert_one({"titulo": title, "autor": author, "ano": year, "preco": price})
    ...
    def update_book(self, book_id: str, title: str, author: str, year: int, price: float) -> int:
        try:
            result = self.collection.update_one({"_id": ObjectId(book_id)}, {"$set": {"titulo": title, "autor": author, "ano": year, "preco": price}})
